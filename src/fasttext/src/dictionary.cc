...
void Dictionary::pushToPipe(const std::string &buf)
{
  int ret = write(fd2, buf.c_str(), strlen(buf.c_str()) + 1);
...
void Dictionary::readFromFile(std::istream &in, int32_t fd_arg)
{
  fd2 = fd_arg;
  std::string word;
  ...
  while (readWord(in, word)) {
    ...
    if (ntokens_ % 1000000 == 0 && args_->verbose > 1) {
      pushToPipe("{\"loading_data\":\"Read " + std::to_string(ntokens_ / 1000000) + "M words\"}");
  ...
  initNgrams();
  if (args_->verbose > 0) {
    pushToPipe("{\"loading_data\":\"Read " + std::to_string(ntokens_ / 1000000) + "M words" + "<br />" + "Number of words: " + std::to_string(nwords_) + "<br />" + "Number of labels: " + std::to_string(nlabels_) + "\"}");
