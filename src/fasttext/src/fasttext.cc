...
#define _PATH_ "/Users/zhouziyang/Documents/LMU/Studium/Informatik/Bachelorarbeit/Repo/Vis-Tuning/src/static/pipe/fasttext.pipe"
#define _SIZE_ 100
//
...
void FastText::openPipe() {
  printf("Pipe is opening...\n");
  fd = open(_PATH_, O_WRONLY);
...
void FastText::pushToPipe(const std::string& buf)
{
  int ret = write(fd, buf.c_str(), strlen(buf.c_str()) + 1);
...
void FastText::printInfo(real progress, real loss, std::ostream& log_stream) {
  ...
  std::string push = "{\"training_progress\":\"Progress: " + std::to_string(prog) + "%, words/sec/thread: " + std::to_string((int)wst) + ", lr: " + std::to_string(lr) + ", avg.loss: " + std::to_string(loss) + ", ETA: " + std::to_string(eta) + "s\"}";
  pushToPipe(push);
  log_stream << std::fixed;
  log_stream << "Progress: ";
  // pushToPipe("progress," + std::to_string(progress));
  log_stream << std::setprecision(1) << std::setw(5) << progress << "%";
  log_stream << " words/sec/thread: " << std::setw(7) << int64_t(wst);
  log_stream << " lr: " << std::setw(9) << std::setprecision(6) << lr;
  log_stream << " avg.loss: " << std::setw(9) << std::setprecision(6) << loss;
  // pushToPipe("eta," + std::to_string(eta));
